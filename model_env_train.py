...
def load_data():
    ...
    proc_scalar_labels=['DeformTemp_C', 'DeformRate','HT1Temp_C','HT1Time_h','HT2Temp_C','HT2Time_h']

    prop_labels = PROP
    # data = data.dropna(subset=prop_labels)

    data_filtered = data[data['Activated'] == 1]
    # Normalize whitespace-only cells to NaN and coerce numeric columns to numbers
    data_filtered = data_filtered.replace(r'^\s*$', np.nan, regex=True)
    ...
    cols_to_numeric = [c for c in cols_to_numeric if c in data_filtered.columns]
    if cols_to_numeric:
        data_filtered[cols_to_numeric] = data_filtered[cols_to_numeric].apply(pd.to_numeric, errors='coerce')

    comp_data = data_filtered[comp_labels].to_numpy()
    proc_bool_data = data_filtered[proc_bool_labels].to_numpy()
    proc_scalar_data = data_filtered[proc_scalar_labels].to_numpy()
    # Combine all processing condition features
    ...
    return comp_data, proc_bool_data, proc_scalar_data, prop_data, elem_feature

def fit_transform(data_tuple: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray,]):
    '''fit and transform the data'''
    comp_data, proc_bool_data, proc_scalar_data, prop_data, elem_feature = data_tuple
    comp_data_scaler, proc_scalar_data_scaler, prop_data_scaler, elem_feature_scaler = \
        [StandardScaler() for _ in range(4)]
    ...
    return (
        (comp_data, proc_bool_data, proc_scalar_data, prop_data, elem_feature,),
        (comp_data_scaler, proc_scalar_data_scaler, prop_data_scaler, elem_feature_scaler,),
    )
...
class CustomDataset(Dataset):
    ...
    def __init__(self, 
                 data_tuple: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray,],
                 scaler: TransformerMixin): # TODO deprecate scaler
        self.data_tuple = data_tuple
        self.scaler = scaler

        self.comp = self.data_tuple[0]
        self.proc_bool = self.data_tuple[1]
        self.proc_scalar = self.data_tuple[2]
        self.prop = self.data_tuple[3]
...
def get_dataloader(data_tuple, batch_size = 16) -> DataLoader:
    ...
    '''
    comp_data, proc_bool_data, proc_scalar_data, prop_data, elem_feature = data_tuple
    dataset = CustomDataset((comp_data, proc_bool_data, proc_scalar_data, prop_data,), None)
    ...
    def _collate_fn(batch):
        comp, proc_bool, proc_scalar, prop, mask = zip(*batch)
...
def train_validate_split(data_tuple, ratio_tuple = (0.95, 0.04, 0.01)):
    ...
    return (comp_train, proc_bool_train, proc_scalar_train, prop_train, elem_feature,), \
            (comp_val_1, proc_bool_val_1, proc_scalar_val_1, prop_val_1, elem_feature,), \
            (comp_val_2, proc_bool_val_2, proc_scalar_val_2, prop_val_2, elem_feature,)
...
def validate(model: CnnDnnModel, data_tuple: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray,]) -> float:
    ...
    comp, proc_bool, proc_scalar, prop, mask, elem_t = next(iter(dl))
    comp = comp.to(device)
    proc_bool = proc_bool.to(device)
    proc_scalar = proc_scalar.to(device)
    prop = prop.to(device)
